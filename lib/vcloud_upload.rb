  ...
  class Client
    ...
    def each_vdc
      ...
      content.each { |e|
        ...
        #each vDC node
        if e.attribute('type').to_s=="application/vnd.vmware.vcloud.vdc+xml"
    ...
    #Create a list of item witch contains the name and the link of all catalog items.
    #
    #@example Print a list of catalogs items
    #  vClient = OpenCloudUP::Client.new('Emma_Example', 'SRo', '123456', 'https://vcd1.example.com')
    #  vClient.login
    #
    #  vClient.each_catalog.each do |item|
    #       puts "Name:  #{item.name}\nLink: #{item.link}"
    #  end
    #
    #  vClient.logout
    ...
    def each_catalog()
      ...
      content.each { |e|
        ...
        #each vDC node
        if e.attribute('type').to_s=="application/vnd.vmware.vcloud.catalog+xml"
